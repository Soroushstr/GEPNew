"""
run_experiment_p2d.py  —  Phase 2d: WeightedGIN + focal loss + 3 layers
Changes on top of Phase 2c:
  - Model: WeightedGIN (more expressive than CustomGAT for this task)
  - num_layers: 2 → 3 (deeper message passing)
  - Loss: FocalLoss with gamma=2.0 (handles extreme imbalance better than
    weighted CE by down-weighting easy negatives)
  - Multi-species leave-one-out training carried forward from Phase 2c
"""
...
from seq_encoder_p2d  import build_dataset, build_vocab, read_fasta
from gnn_models       import WeightedGIN
from pipeline_v_p2d  import train, test
...
CONFIG = {
    ...
    # --- Model architecture (Phase 2d changes) ---
    "emb_dim":    128,
    "hidden_dim": 128,
    "num_layers": 3,        # increased from 2 → 3
    "dropout":    0.2,
    "pool":       "mean",
    "train_eps":  True,
    ...
    "use_focal_loss":         True,   # Phase 2d: focal loss
    "focal_gamma":            2.0,
    "use_scheduler":          True,
    ...
    "experiment_name": "p2d_wgin_focal",
}
...
def main():
    ...
    for test_species in CONFIG["test_species"]:
        ...
        # ---- Load and concatenate training graphs ----
        ...
        model    = WeightedGIN(**model_config).to(device)
        n_params = sum(p.numel() for p in model.parameters())
        print(f"\n  Model: WeightedGIN  |  params: {n_params:,}")
        ...
        test_model = WeightedGIN(**torch.load(config_path)).to(device)
        results = test(
            graphs=test_graphs,
            model=test_model,
            model_path=model_path,
    ...
    ax.set_ylabel("MCC")
    ax.set_title("Cross-species MCC  (WeightedGIN + FocalLoss + leave-one-out)")
    ax.set_ylim(-1.0, 1.0)
