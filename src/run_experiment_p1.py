"""
run_experiment_p1.py  —  Phase 1: Fix cross-species threshold
Fix: cross-species test() calls now use threshold=0.5 instead of the
elegans-tuned threshold (which was 0.95 and caused zero recall on all
other species). The sanity check on the train species still uses the
saved optimal threshold.
"""
...
CONFIG = {
    ...
    # --- Phase 1 fix: use threshold=0.5 for cross-species tests ---
    "cross_species_threshold": 0.5,

    # --- Output ---
    "results_dir":      os.path.join(_BASE_DIR, "results"),
    "experiment_name":  "p1_threshold_fix",
}
...
def main():
    ...
    print(f"  Using fixed threshold={CONFIG['cross_species_threshold']} for all cross-species tests.")
    all_metrics = []
    ...
    for species in CONFIG["test_species"]:
        ...
        # Phase 1 fix: use threshold=0.5 instead of the elegans-tuned threshold
        results = test(
            graphs=test_graphs,
            model=test_model,
            model_path=model_path,
    ...
    # ------------------------------------------------------------------ sanity check on train species
    print_separator(f"Sanity check — train species: {CONFIG['train_species']}", char="-")
    print(f"  Using saved optimal threshold from training checkpoint.")
    ...
    # Sanity check: use the saved threshold (threshold=None means use checkpoint value)
    sanity_results = test(
