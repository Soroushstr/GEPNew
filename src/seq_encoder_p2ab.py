"""
seq_encoder_p2ab.py  —  Phase 2a+2b version
Change: build_dataset() now exposes the `bidirectional` parameter so
callers can pass bidirectional=True to model both DNA strands.
"""
...
def build_dataset(fasta_path, labels_path, k, vocab=None, bidirectional=False):
    print("########## Start building graphs from seqs...")
    ...
    for seq, lab, gid in zip(seqs, labels, gene_ids):
        g = seq_to_graph(seq, k, vocab, bidirectional=bidirectional)
        if g is None:
            continue
